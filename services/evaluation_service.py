"""Servicio de aplicacion para evaluacion RAGAS simplificada."""
...
class EvaluacionRAGASService:
    def __init__(self, repo_evaluaciones: Any, repo_consultas: Any) -> None:
        self.repo_evaluaciones = repo_evaluaciones
    ...
    async def evaluar_respuesta_rag(
        ...
        id_consulta = to_int(id_consulta, "id_consulta")
        if id_consulta <= 0:
    ...
    async def obtener_historial_evaluaciones(
        ...
        id_usuario = to_int(id_usuario, "id_usuario")
        if id_usuario <= 0:
